...
var newsText = "第十二屆奧美林宗緯紅領帶實習計畫開始報名";
var header_links = [
...
function scrollToTop () {
	time = Math.abs((scrollTop.get() - 0) * .0005) < .5 ? .5 : Math.abs((scrollTop.get() - 0) * .0005);
		...
		for (i = 0; i < menu_links.length; i++) {
			for (j = 0; j < menu_links[i].length; j++) {
				if (menu_links[i][j].name == tag[1]) {
					time = Math.abs((scrollTop.get() - menu_links[i][j].top) * .0005) < .5 ? .5 : Math.abs((scrollTop.get() - menu_links[i][j].top) * .0005);
					if (isPageUnload) {
