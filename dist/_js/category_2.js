	...
	success: function (data) {
		...
		for (var countryObj in menu) {
			for (var i = 0; i < menu[countryObj].length; i++) {
				sendTime = {
					start: new Date(menu[countryObj][i]['startTime']),
					end: new Date(menu[countryObj][i]['endTime']),
				};
				timeoutMsg =
					'報名的時間在 ' +
					sendTime.start.getFullYear().toString() +
					' / ' +
					(sendTime.start.getMonth() + 1).toString() +
					' / ' +
					sendTime.start.getDate() +
					' (' +
					sendTime.start.getHours() +
					':' +
					sendTime.start.getMinutes() +
					':' +
					sendTime.start.getSeconds() +
					') 至 ' +
					sendTime.end.getFullYear().toString() +
					' / ' +
					(sendTime.end.getMonth() + 1).toString() +
					' / ' +
					sendTime.end.getDate() +
					' (' +
					sendTime.end.getHours() +
					':' +
					sendTime.end.getMinutes() +
					':' +
					sendTime.end.getSeconds() +
					')';
				checkTime();
			}
		}
